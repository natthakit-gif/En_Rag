...
function ChatPage({ user, initialPrompt }) {
    ...
    // เพิ่มการตรวจสอบ location.state เพื่อล้างประวัติการสนทนา
    useEffect(() => {
        if (location.state && location.state.clearHistory) {
            // ล้างข้อมูลการสนทนาเก่า
            setMessages([]);
            try {
                localStorage.removeItem("chatHistory");
                QAService.saveConversation([]).catch((err) =>
                    console.error("Failed to clear conversation:", err)
                );
            } catch (error) {
                console.error("Failed to clear conversation:", error);
            }
            // ล้าง state เพื่อไม่ให้ล้างข้อมูลซ้ำเมื่อมี re-render
    ...
    // ปรับปรุงการจัดการการเลื่อน
    useEffect(() => {
        const container = messagesContainerRef.current;
        if (!container) return;
        
        const handleScroll = () => {
            // ตรวจสอบว่าผู้ใช้เลื่อนใกล้ด้านล่างหรือไม่ (ภายในระยะ 200px)
    ...
    // ปรับปรุงฟังก์ชัน scrollToBottom ให้ทำงานได้ดีขึ้น
    ...
    const handleFaqClick = async (faqItem) => {
        ...
        try {
                ...
                setTimeout(() => {
                    ...
                    if (i === words.length - 1) {
                        setTimeout(() => {
                            ...
                            // บันทึกลงใน localStorage ด้วย
                            localStorage.setItem("chatHistory", JSON.stringify([...messages, newUserMessage, assistantMessage]));
                        }, 500);
                    }
                }, i * 50);
            }
            ...
            // บันทึกลงใน localStorage ด้วย
            localStorage.setItem("chatHistory", JSON.stringify([...messages, newUserMessage, errorMessage]));
        } finally {
            setIsLoading(false);
        }
    };
    ...
    // ปรับปรุงฟังก์ชัน handleClearConversation ไม่ให้แสดง confirm dialog
    ...
    return (
        <SideBar user={user} activePage="chat">
            <div className="flex flex-col min-h-screen bg-base-100">
                ...
                <div className="bg-base-200 p-2 flex justify-end">
                    <button
                ...
                {/* ส่วนของข้อความสนทนา ปรับปรุงให้ใช้ className overflow-y-auto ด้วย */}
                <div 
                    className="flex-1 overflow-y-auto p-4" 
                    ref={messagesContainerRef} 
                    style={{ 
                        height: getMessagesContainerHeight(), 
                        minHeight: "250px",
